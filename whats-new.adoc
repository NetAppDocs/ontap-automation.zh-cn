...
ONTAP 9.18.1 版本继续扩展ONTAP REST API 的功能，新增了 11 个 API 调用。此次改进主要集中在安全性方面，但也包括与存储管理相关的改进。

安全性:: ONTAP 9.18.1 REST API 引入了两项主要的安全增强功能。您可以使用新的集群网络 API 端点来显示和更新集群网络安全配置。集群网络证书管理功能强大。此外，您还可以显示和更新 NVLog 流量的 HA 网络的安全配置。
