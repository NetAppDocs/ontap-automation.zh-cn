...
ONTAP 9.19.1 版本包括几个更新，这些更新继续增强和改进 ONTAP REST API。

安全性:: 已添加一项 `whoami`新功能，用于检索已登录用户的用户名、角色和权限信息。Web 身份验证（WebAuthn）功能也得到了增强。IPsec 已更新，可检索与集群网络一起使用的策略和安全关联。
SAN:: LUN 映射管理已更新，具有改进的更新功能。
